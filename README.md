...
**Latest TASK-TTS-004C2 manual probe (2026-06-18):** Edge-TTS Manual Dependency Probe / Chinese Audio Output **DONE - EDGE-TTS MANUAL AUDIO OUTPUT SUCCESS / LISTENING PENDING**. With explicit approval, `edge-tts` was installed into `backend\.venv` only for the manual provider probe (`edge-tts==7.2.8` plus transitive packages). Metadata-only probe now reports `available=true`, `reason=optional_dependency_present`, `synthesisStatus=metadata_only`, `voice=zh-TW-HsiaoChenNeural`, and `audioGenerated=false`; metadata-only mode still does not send text to Microsoft Edge TTS service. Optional audio probe with `--allow-audio-output` generated an ignored local MP3 (`audioGenerated=true`, `audioBytes=30240`, `synthesisLatencyMs=1613`, `reason=edge_tts_success`) under `outputs/tts_provider_probe/20260618/audio/`. The MP3 was not played by the app or committed. `edge_tts` remains network/cloud-ish, optional, not default, and not selected as runtime; Chinese listening verdict is pending manual playback.
...
| TTS provider probe | ✅ TASK-TTS-004C IMPLEMENTED - EDGE-TTS OPTIONAL PROBE READY / CHINESE AUDIO VALIDATION PENDING；metadata-only default；no runtime wiring/playback/dependency install |
...
> TASK-TTS-004C2：Edge-TTS Manual Dependency Probe / Chinese Audio Output 完成 manual probe (2026-06-18)，狀態為 DONE - EDGE-TTS MANUAL AUDIO OUTPUT SUCCESS / LISTENING PENDING。經明確批准後只在 `backend\.venv` 安裝 optional `edge-tts` probe dependency；metadata-only PASS，optional audio PASS 並產生 ignored local MP3。未新增 runtime wiring、播放、自動說話、default provider selection，未 commit 生成音訊或報告；中文聽感仍待使用者手動播放判定。
