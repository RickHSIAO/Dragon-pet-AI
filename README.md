...
**Latest TASK-TTS-004C3 implementation (2026-06-18):** Edge-TTS Voice / Rate Tuning Probe **IMPLEMENTED - EDGE-TTS TUNING PROBE READY / MANUAL LISTENING PENDING**. This is docs-only because `scripts/tts_provider_probe.py` already supports `--edge-tts-voice`, `--edge-tts-rate`, and `--edge-tts-pitch`. The tuning matrix covers `zh-TW-HsiaoChenNeural` at `+0%`, `-10%`, and `-15%`; `zh-TW-HsiaoYuNeural` at `+0%`, `-10%`, and `-15%`; `zh-CN-XiaoxiaoNeural` at `+0%` and `-10%`; and optional `+2Hz`/`+5Hz` pitch checks only after a promising voice/rate is found. Baseline remains `zh-TW-HsiaoChenNeural +0% +0Hz = 6/10`: understandable Chinese, slightly fast, too general/Taiwanese, weak Christina/anime fit. Decision rule: `>= 7/10` can be an acceptable temporary Chinese provider candidate; `< 7/10` stays fallback/debug only; regardless of score, edge-tts is not final long-term Christina voice unless explicitly selected later. Runtime TTS remains disabled/mock-only. No `/chat` integration, playback, Pet Window playback, auto-speaking, dependency/default-runtime change, generated audio/report commit, STT, Conversation Mode, Owner Voice, or schema behavior change was added.
...
| TTS provider probe | ✅ TASK-TTS-004C3 IMPLEMENTED - EDGE-TTS TUNING PROBE READY / MANUAL LISTENING PENDING；voice/rate/pitch matrix documented；no runtime wiring/playback/default-provider change |
...
> TASK-TTS-004C3：Edge-TTS Voice / Rate Tuning Probe 完成 docs-only workflow (2026-06-18)，狀態為 IMPLEMENTED - EDGE-TTS TUNING PROBE READY / MANUAL LISTENING PENDING。既有 `scripts/tts_provider_probe.py` 已支援 `--edge-tts-voice`、`--edge-tts-rate`、`--edge-tts-pitch`，所以未新增 batch mode 或 runtime code。調音矩陣涵蓋 HsiaoChen / HsiaoYu / Xiaoxiao、多個慢速 rate 與可選 pitch 微調；人工聽感欄位與 `>= 7/10` 臨時 provider 判定規則已記錄。未新增 runtime wiring、播放、自動說話、default provider selection，未 commit 生成音訊或報告。
