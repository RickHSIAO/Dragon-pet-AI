...
**Latest TASK-CONV-002 update (2026-06-11):** Conversation Mode Turn Lifecycle Visibility / Missing Turn Diagnostics **IMPLEMENTED - AUTOMATED DIAGNOSTICS SMOKE PASS / NEEDS WINDOWS RUNTIME 4-TURN SMOKE**. Voice Diagnostics now shows bounded session-only `turn#N lifecycle` rows for recorded/finalized/queued/dropped/STT/chat/drain states using safe scalar fields only. No STT default, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema, IPC, Pet Window, Output Queue, raw audio persistence, path exposure, transcript exposure, centroid exposure, or embedding exposure change.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Continue pending TASK-CONV-002 Windows runtime 4-turn smoke plus TASK-CONV-001 and TASK-AUDIO-001 Windows runtime smoke. TASK-PERSONA-001 is closed; keep debug fallback repair enabled unless a future runtime tone task deliberately replaces it. |
