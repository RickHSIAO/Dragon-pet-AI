...
**最新本地狀態（2026-06-02）：** TASK-240 Christina Desktop Pet Cutout Stage Foundation 已完成，狀態為 **DONE - WINDOWS VISUAL SMOKE PASS / DONE - PASS**。透明 cutout shell；角色圖片 / avatar container 為主要 drag 區；底部 Chat / Full App / Mic / Menu 預設隱藏，hover / focus-within 才顯示且可操作；close X 永久隱藏，Hide Pet Window 保留在 Menu；局部 crimson glow + oval stage pad 綁定於角色；無 HTML / JS / IPC / @keyframes 更動。pet-renderer-smoke.js 285 checks PASS（22 TASK-240 tests）。Windows visual smoke PASS (2026-06-02)：11 項確認項目全部 PASS。未 commit、未 push。
...
| Full App chat UX | ✅ TASK-236 DONE；TASK-237 docs-only modularization plan complete；TASK-238 DONE - WINDOWS VISUAL SMOKE PASS；TASK-239 DONE - WINDOWS VISUAL SMOKE PASS；TASK-240 DONE - WINDOWS VISUAL SMOKE PASS — 搜尋/高亮、未讀提示、匯出、時間戳、日期分隔線、清除確認、empty state、Undo Clear Chat、單則訊息刪除/復原、右鍵訊息操作 (viewport clamp + a11y)、最後 user message 編輯/重新送出、互動事件 log、reaction hint 層、reaction preview UI、expression suggestion 鏡像至 Pet Window、expression mirror 300ms cooldown/debounce、reaction bubble 鏡像至 Pet Window、companion behavior policy decision preview、character state diagnostics preview、collapsible diagnostics drawer、output queue module extraction、diagnostics drawer module extraction、Pet Window cutout stage + hover dock / chrome reduction |
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | TASK-241 Full App Voice Input Button |
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 完整階段開發路線圖；目前 TASK-240 Christina Desktop Pet Cutout Stage Foundation DONE - WINDOWS VISUAL SMOKE PASS，下一步 TASK-241 |
| [docs/TASKS.md](docs/TASKS.md) | 完整任務歷史記錄；最新記錄為 TASK-240 Christina Desktop Pet Cutout Stage Foundation，DONE - WINDOWS VISUAL SMOKE PASS / DONE - PASS |
