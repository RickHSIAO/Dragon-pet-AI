...
**Latest local status (2026-06-11):** TASK-STT-003 Runtime STT Model Override / base-small Candidate Smoke **IMPLEMENTED - AUTOMATED SMOKE PASS / NEEDS WINDOWS RUNTIME CANDIDATE SMOKE**. Runtime STT still defaults to faster-whisper `tiny`, but backend startup now accepts the short safe override `DRAGON_STT_MODEL=base|small|tiny` for Manual Mic and Conversation Mode candidate testing; legacy `DRAGON_PET_STT_MODEL` remains supported. Diagnostics now include requested/resolved model, model source, model env, model fallback reason, model load status/error, provider source/load/fallback, and final transcript path. TASK-STT-002 safe samples showed `tiny` is not acceptable, `base` is the first runtime candidate by current CER/latency balance, and `small` remains a slower quality candidate needing more Windows smoke. No committed default switch, Conversation Mode queue change, Owner Voice Gate behavior change, `/stt/transcribe` or `/chat` schema break, raw audio persistence, or aggressive transcript rewrite was added.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Run Windows Manual Mic and Conversation Mode runtime smoke with `DRAGON_STT_MODEL=base`, then repeat targeted checks with `DRAGON_STT_MODEL=small` for names and mixed Chinese/English technical terms. Keep committed default at `tiny` until candidate smoke confirms quality, latency, diagnostics, TASK-AUDIO-001 pre-roll, TASK-CONV-001 graceful drain, and Owner Voice non-blocking behavior. |
