...
**Latest TASK-CONV-005 implementation (2026-06-12):** Conversation Mode Long Session Stability Smoke **IMPLEMENTED - AUTOMATED LONG SESSION DIAGNOSTICS SMOKE PASS / NEEDS WINDOWS 5-10 MIN CONVERSATION SMOKE**. `apps/desktop/scripts/conversation-long-session-smoke.js` verifies the renderer diagnostics source shape and a synthetic 12-turn lifecycle fixture for longer Conversation Mode sessions. It guards bounded queue max `4`, queue pressure/full visibility, `drain_complete`, distinct `queue_full` usable-audio overflow vs `empty_artifact` before-queue drops, Owner Voice dry-run visibility, candidate WAV deletion facts, and the copyable long-session summary format. This is diagnostics/checklist coverage only: no runtime behavior, STT default, runtime model selector behavior, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit was added.
