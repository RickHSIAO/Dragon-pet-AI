...
**Latest local status (2026-06-04):** TASK-SEC-004 Tool Permission / User Confirmation Policy **DONE - DOCS-ONLY TOOL PERMISSION POLICY**. Added `docs/security/TOOL_PERMISSION_POLICY.md` with T0-T6 tool tiers, allowed/forbidden actions, confirmation and preview requirements, logging and LLM-context handling, failure behavior, outbound/local-file/URL safety rules, Owner Voice Gate tool boundary, prompt-injection interaction rules, audit/logging rules, and future implementation checklist. No backend runtime, frontend runtime, IPC, Manual Mic, Conversation Mode, STT, `/chat`, Pet Window, Output Queue, Diagnostics behavior, owner voice runtime wiring, or actual tool execution behavior changed.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | TASK-SEC-004 Tool Permission Policy **DONE - docs-only** → TASK-SEC-005 Phishing / Link Safety Warning Layer → TASK-266 Owner Voice Gate Manual Mic Dry-run Policy |
