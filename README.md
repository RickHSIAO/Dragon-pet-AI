...
**Latest local status (2026-06-05):** TASK-STT-001 Chinese STT Punctuation Restoration / Transcript Readability **IMPLEMENTED - CONSERVATIVE LOCAL PUNCTUATION LAYER**. Backend STT ok-path now preserves `rawTranscript`, applies existing safe-dictionary correction to `correctedTranscript`, then applies deterministic text-only punctuation restoration to produce `punctuatedTranscript` / `finalTranscript`; `transcript` is the final transcript used by Manual Mic textarea and Conversation Mode `/chat`. Renderer Voice Diagnostics now shows raw, corrected, punctuated, final transcript previews plus `punctuationApplied`, `punctuationMode`, and `punctuationReason` via `textContent`. The first version adds only conservative terminal CJK punctuation when text is long enough and lacks sentence punctuation; it does not paraphrase, invent words, replace aliases outside the existing correction map, use an LLM, add IPC, change `/stt/transcribe` request schema, change `/chat` schema, change Pet Window / Output Queue behavior, change voice recording, or alter Owner Voice Gate dry-run / hard-gate policy.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | TASK-STT-001 implemented; run requested validation and then consider live Windows voice smoke for punctuation readability. TASK-270 remains DONE - Windows runtime lifecycle smoke PASS; Conversation Mode score/calibration quality remains a separate future follow-up before any hard gate |
