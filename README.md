...
**Latest TASK-TTS-004B2 implementation (2026-06-18):** VOICEVOX Synthesis Timeout / Retry Hardening **IMPLEMENTED - VOICEVOX TIMEOUT HARDENING SMOKE PASS / NEEDS MANUAL AUDIO RETRY**. `scripts/tts_provider_probe.py` now exposes `--voicevox-timeout-sec` (default `30`) and `--voicevox-retries` (default `1`) for the manual `voicevox_server` probe, with stage diagnostics for `voicevoxStage`, version/speakers/audio_query/synthesis latency, timeout seconds, retry count, and last exception class/message. This follows the manual Windows result where VOICEVOX Engine `0.25.2` metadata and speaker discovery worked, but optional audio timed out before quality could be judged. After hardening, the local optional retry command reached `voicevox_success` and generated a WAV under ignored `outputs/tts_provider_probe/YYYYMMDD/audio/`; pronunciation quality still requires manual listening. Default metadata-only mode still does not call synthesis or write audio; optional WAV generation remains gated behind `--allow-audio-output`, localhost-only, and playback-free. No runtime TTS wiring, `/chat` integration, playback, auto-speaking, dependency/install, generated audio/report commit, STT default/model selector change, Conversation Mode queue/backpressure change, Owner Voice hard-gate change, or schema change was added.
...
| [docs/TTS_ARCHITECTURE.md](docs/TTS_ARCHITECTURE.md) | TASK-TTS-001/TASK-TTS-004B provider-neutral TTS architecture：text normalization、disabled queue diagnostics、mock provider skeleton、local provider probe/review phase、VOICEVOX manual localhost probe、Conversation Mode and Pet Window integration、diagnostics、phased plan |
| [docs/TTS_PROVIDER_RESEARCH.md](docs/TTS_PROVIDER_RESEARCH.md) | TASK-TTS-001/TASK-TTS-004B provider research：local/offline candidates、implemented mock provider boundary、candidate probe/review table、VOICEVOX manual workflow、evaluation criteria、voice data rules、future singing boundary |
...
| `docs/TTS_ARCHITECTURE.md` | TASK-TTS-001/TASK-TTS-004B provider-neutral local TTS architecture, mock provider skeleton, disabled queue diagnostics, local provider probe/review phase, VOICEVOX manual localhost probe, text normalization, Conversation Mode, Pet speaking state, and phased implementation plan |
| `docs/TTS_PROVIDER_RESEARCH.md` | TASK-TTS-001/TASK-TTS-004B local TTS provider research boundaries, candidate categories, implemented mock provider non-quality boundary, probe/review status table, VOICEVOX manual workflow, evaluation criteria, voice data rules, and future provider path |
...
> TASK-TTS-004B2：VOICEVOX Synthesis Timeout / Retry Hardening 完成 probe hardening (2026-06-18)，狀態為 IMPLEMENTED - VOICEVOX TIMEOUT HARDENING SMOKE PASS / NEEDS MANUAL AUDIO RETRY。新增 `--voicevox-timeout-sec`、`--voicevox-retries`、stage latency fields、`voicevoxStage`、`retryCount`、last exception diagnostics。Default metadata-only 不呼叫 audio_query/synthesis；optional audio 仍需 `--allow-audio-output`，且只允許 localhost、不播放、不接 runtime。
