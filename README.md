...
**Latest TASK-STT-006A implementation (2026-06-12):** Backend STT Model Evaluation Report **IMPLEMENTED - AUTOMATED REPORT SMOKE PASS / NEEDS WINDOWS AUDIO SAMPLE EVALUATION**. Added `scripts/stt_model_evaluation_report.py` to compare local user-provided audio across `tiny`, `base`, and `small` through the runtime STT service path using per-request model selection. Reports write JSON under `outputs/stt_model_evaluation/YYYYMMDD/` with safe redacted sample basenames, latency/RTF, transcript raw/final fields, no-speech guard diagnostics, provider/model load metadata, fallback, and per-model status counts. This is data collection only: no recommended model, scoring decision, subjective AI explanation, frontend comparison panel, committed audio sample, or generated report is added. Runtime default remains `tiny`; `base` and `small` remain runtime candidates only.
