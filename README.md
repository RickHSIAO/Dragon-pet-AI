...
**Latest local status (2026-06-05):** TASK-AUDIO-001 Capture Start Latency Measurement / Conversation Pre-roll Buffer **IMPLEMENTED - AUTOMATED RENDERER SMOKE PASS / NEEDS WINDOWS RUNTIME SMOKE**. Voice Diagnostics now tracks safe per-capture timing (`captureRequestedAt`, `mediaStreamReadyAt`, `recorderStartRequestedAt`, `recorderStartedAt`, `firstChunkAt`, `vadTriggeredAt`, `recordingFinalizedAt`, `captureReadyLatencyMs`, `firstChunkLatencyMs`). Manual Mic no longer claims active recording before `MediaRecorder.start()` succeeds; it shows a short preparing state first. Conversation Mode now keeps a bounded in-memory PCM pre-roll buffer while listening (`500ms`) and prepends it to the utterance when VAD triggers, then clears the rolling buffer. No raw audio is persisted, no pre-roll content/path is exposed, and no STT model, transcript rewrite, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema, IPC, Pet Window, or Output Queue behavior was changed. TASK-CONV-001 queue/graceful-drain remains implemented but user-facing DONE acceptance is blocked until Windows capture/pre-roll smoke and a later STT quality benchmark confirm capture and recognition quality.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Run Windows live TASK-AUDIO-001 smoke with actual audio: play back Manual Mic and Conversation Mode recordings, confirm sentence starts are preserved, confirm pre-roll does not prepend stale previous utterance audio, confirm graceful Stop still drains queued turns, and confirm no temp audio remains after Stop. Then run a separate STT quality benchmark/model task before marking TASK-CONV-001 DONE. TASK-270 remains DONE - Windows runtime lifecycle smoke PASS; Conversation Mode score/calibration quality remains a separate future follow-up before any hard gate |
