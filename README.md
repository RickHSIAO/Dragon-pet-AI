...
**最新本地狀態（2026-06-01）：** TASK-222 Interactive Companion Architecture Checkpoint 已完成，狀態為 **IMPLEMENTED - DOCS CHECKPOINT / NO WINDOWS SMOKE REQUIRED**。這是 docs-only architecture checkpoint，整理 TASK-214 至 TASK-221 的互動陪伴架構：interaction event → reaction hint → expression suggestion → behavior decision → expression mirror / reaction bubble → Pet Window expression / fixed bubble。新增 [docs/INTERACTIVE_COMPANION_ARCHITECTURE.md](docs/INTERACTIVE_COMPANION_ARCHITECTURE.md)，記錄資料流、layer responsibility、IPC inventory、安全邊界、目前 behavior examples、smoke coverage、已知未實作範圍與下一階段建議。未修改程式碼、backend、`/chat` schema、chat history format、IPC、Pet Window runtime、Full App renderer runtime、smoke tests、assets、Ollama / Provider runtime；未 commit、未 push。
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 完整階段開發路線圖；目前 TASK-222 interactive companion architecture checkpoint 已完成 |
| [docs/TASKS.md](docs/TASKS.md) | 完整任務歷史記錄；最新記錄為 TASK-222 docs-only architecture checkpoint |
| [docs/INTERACTIVE_COMPANION_ARCHITECTURE.md](docs/INTERACTIVE_COMPANION_ARCHITECTURE.md) | TASK-222 互動陪伴架構 checkpoint：data flow、layer responsibility、IPC inventory、安全邊界與下一階段 |
...
| `docs/INTERACTIVE_COMPANION_ARCHITECTURE.md` | TASK-222 互動陪伴架構 checkpoint |
