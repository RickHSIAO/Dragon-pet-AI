...
**Latest local status (2026-06-11):** TASK-STT-002 Chinese STT Quality Baseline / Model Comparison **IMPLEMENTED - LOCAL QUALITY PROBE / NO RUNTIME DEFAULT CHANGE**. Added `scripts/stt_quality_probe.py` to compare local faster-whisper `tiny`, `base`, `small`, and optional `medium` against explicit WAV/reference samples or a private manifest. The probe reports raw/corrected/punctuated/final transcripts, character error rate, latency, audio duration, real-time factor, repeated-token warnings, empty transcript warnings, and capture-vs-model source grouping. Missing models are skipped by default unless `--allow-download` is passed. No runtime STT default changed (`tiny` remains default), no Conversation Mode queue behavior changed, no Owner Voice Gate behavior changed, no `/stt/transcribe` or `/chat` schema changed, and no aggressive transcript rewriting was added. TASK-CONV-001 user-facing DONE acceptance still depends on Windows capture/pre-roll smoke plus STT quality evidence.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Run Windows live TASK-AUDIO-001 smoke with actual audio, then run TASK-STT-002 with safe non-private WAV samples to compare `tiny` / `base` / `small` before deciding the next runtime candidate. Do not mark TASK-CONV-001 DONE until capture/pre-roll and STT quality evidence both pass. TASK-270 remains DONE - Windows runtime lifecycle smoke PASS; Conversation Mode score/calibration quality remains a separate future follow-up before any hard gate |
