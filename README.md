...
**Latest TASK-CONV-002 closeout (2026-06-11):** Conversation Mode Turn Lifecycle Visibility / Missing Turn Diagnostics **DONE - WINDOWS RUNTIME 4-TURN LIFECYCLE SMOKE PASS**. Windows runtime smoke with actual Conversation Mode audio and `DRAGON_STT_MODEL=base` confirmed `turn#1` through `turn#4` completed with `stt=success` and `chat=sent`, final state was capture `off` / processing `idle` / pending `0/2` / `activeTurnId=0` / `stopMode=drain_complete`, and Owner Voice dry-run stayed non-blocking with candidate WAV temporary/deleted. Diagnostics also surfaced `turn#5 dropped reason=queue_full`; track that separately as TASK-CONV-003 queue backpressure / extra dropped turn investigation. No STT default, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema, IPC, Pet Window, Output Queue, raw audio persistence, path exposure, transcript exposure, centroid exposure, or embedding exposure change.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Continue pending TASK-CONV-001 and TASK-AUDIO-001 Windows runtime smoke; consider TASK-CONV-003 queue backpressure / extra dropped turn investigation from the TASK-CONV-002 runtime smoke. TASK-PERSONA-001 is closed; keep debug fallback repair enabled unless a future runtime tone task deliberately replaces it. |
