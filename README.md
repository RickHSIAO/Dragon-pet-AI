...
**最新本地狀態（2026-06-01）：** TASK-224 Character State Preview Polish / Diagnostics 已完成 automated smoke 與 Windows visual smoke PASS，狀態為 **DONE - WINDOWS VISUAL SMOKE PASS / DONE - PASS**。Full App renderer-only 將 interaction preview 收斂為安全 diagnostics block：`Reaction: <hint> · Suggestion: <expression>` / `Decision: <action> · State: <mood>/<attention>/<energy> · Level: <recentInteractionLevel>`。Preview fallback 固定為 `Reaction: none · Suggestion: neutral` / `Decision: none · State: neutral/idle/calm · Level: none`，只讀 allowlisted renderer state，不顯示 raw JSON、event payload、raw user message text、backend/provider diagnostics，也不進入 chat history 或 copy/export transcript。TASK-224 沒有新增 IPC，保留 TASK-218 / TASK-220 窄 channel；不改 backend、`/chat` schema、history format、Pet Window runtime、Pet Bubble、expression mirror、reaction bubble mirror、Ollama / Provider runtime、assets 或 persistence；不呼叫 `/chat`、不寫 history、不觸發 TTS、不主動發話。Windows visual smoke PASS：基本啟動 Reaction/Suggestion 與 Decision/State/Level 顯示正常且 Pet Window 正常；送出訊息、Delete/Undo、Edit last user、Clear Chat、Focus 的 preview/state/level 均正確；Diagnostics 無 undefined/null/[object Object]/NaN/raw JSON/user text；無新增 IPC side-effect、額外 TTS、額外 `/chat`、history/copy/export 污染，Pet Window 表情與 reaction bubble 行為維持正常。
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 完整階段開發路線圖；目前 TASK-224 已完成 automated smoke 與 Windows visual smoke PASS |
