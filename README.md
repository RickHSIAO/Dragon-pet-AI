...
**最新本地狀態（2026-06-01）：** TASK-223 Character State Layer Foundation 已完成 automated smoke 與 Windows visual smoke PASS，狀態為 **DONE - WINDOWS VISUAL SMOKE PASS / DONE - PASS**。Full App renderer-only 新增純本地 character state layer，從 behavior decision / reaction hint / expression suggestion / recent interaction events 推導 `attention/energy/mood/recentInteractionLevel/source/reason/ts`，ring buffer max 20。Preview 顯示 `Reaction: <hint> · Suggestion: <expression> · Decision: <action> · State: <mood>/<attention>/<energy>`。Character state 只做本地 state summary / preview，不送 Pet Window、不新增 IPC、不改 Pet Window runtime、Pet Bubble、expression mirror、reaction bubble mirror、backend、`/chat` schema、history format、Ollama / Provider runtime、assets 或 persistence；不呼叫 `/chat`、不寫 history、不觸發 TTS、不保存 raw message text。Windows visual smoke PASS：基本啟動 none/neutral/none/state neutral/idle/calm 且 Pet Window 正常；送出訊息 user_active/focused/mirror_expression_and_bubble/state focused/active/attentive；Delete / Undo message_management/neutral/mirror_expression_and_bubble/state neutral/managing/calm；Edit correction/annoyed/mirror_expression_and_bubble/state annoyed/correcting/attentive；Clear reset/neutral/mirror_expression_and_bubble/state neutral/reset/calm；Focus attention_returned/happy/mirror_expression_and_bubble/state happy/returned/lively；連續互動無 undefined/null/[object Object]/raw JSON；無新增 IPC side-effect、額外 TTS、額外 `/chat`、history/copy/export 污染，Pet Window 表情與 reaction bubble 行為維持正常。
...
| Full App chat UX | ✅ TASK-223 DONE - WINDOWS VISUAL SMOKE PASS — 搜尋/高亮、未讀提示、匯出、時間戳、日期分隔線、清除確認、empty state、Undo Clear Chat、單則訊息刪除/復原、右鍵訊息操作 (viewport clamp + a11y)、最後 user message 編輯/重新送出、互動事件 log、reaction hint 層、reaction preview UI、expression suggestion 鏡像至 Pet Window、expression mirror 300ms cooldown/debounce、reaction bubble 鏡像至 Pet Window、companion behavior policy decision preview、character state summary preview |
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 完整階段開發路線圖；目前 TASK-223 已完成 automated smoke 與 Windows visual smoke PASS |
| [docs/TASKS.md](docs/TASKS.md) | 完整任務歷史記錄；最新記錄為 TASK-223 Character State Layer Foundation automated smoke 與 Windows visual smoke PASS |
| [docs/INTERACTIVE_COMPANION_ARCHITECTURE.md](docs/INTERACTIVE_COMPANION_ARCHITECTURE.md) | TASK-222/223 互動陪伴架構 checkpoint：data flow、layer responsibility、IPC inventory、安全邊界、Character State Layer 與下一階段 |
...
| `docs/INTERACTIVE_COMPANION_ARCHITECTURE.md` | TASK-222/223 互動陪伴架構 checkpoint |
