...
**Latest TASK-STT-006B implementation (2026-06-12):** Deterministic STT Model Scoring **IMPLEMENTED - AUTOMATED SCORING SMOKE PASS / NEEDS WINDOWS SCORING REPORT SMOKE**. Added `scripts/stt_model_scoring_report.py` to read a TASK-STT-006A evaluation JSON and write deterministic runtime-suitability scores under `outputs/stt_model_scoring/YYYYMMDD/`. Profiles `manual_mic`, `conversation`, and `balanced` combine reliability, speed, speech evidence, transcript signal, hallucination risk, and fallback penalties into 0-100 model scores plus deterministic reason codes and decision limits. This is not transcript accuracy/WER scoring without reference transcripts. No LLM/AI explanation, frontend comparison panel, runtime model auto-switch, `/chat` or mood schema change, Owner Voice hard-gate change, generated scoring report commit, or STT default change; committed default remains `tiny`, and `base`/`small` remain runtime candidates only.
