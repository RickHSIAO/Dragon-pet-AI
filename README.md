...
**Latest TASK-PERSONA-002 implementation (2026-06-12):** Christina General Tone Sanitizer for Non-Debug Replies **IMPLEMENTED - SECOND PROMPT/REPAIR SMOKE PASS / NEEDS WINDOWS GENERAL TONE RE-SMOKE**. Added positive non-debug tone guidance so Christina stays proud, teasing, witty, and protective without contempt, hostile unclear-input dismissal, repeated humiliation, comparative devaluation, threat/intimidation phrasing, or waste-time hostility. The LLM success path applies the existing TASK-PERSONA-001 debug repair first, then a narrow general repair for known harsh fragments, unclear-input hostility, repeated humiliation, likely garbled-STT cases, comparative devaluation, threats, and waste-time hostility. Safe tsundere lines are preserved. Second-pass prompt/chat tests, py_compile, and Electron smoke passed; full backend pytest timed out after 364s in the local command wrapper. No `/chat` schema, mood schema, STT default, Owner Voice hard gate, renderer IPC, provider runtime, or runtime model change.
