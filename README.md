...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | TASK-STT-004 is closed after Windows runtime no-speech smoke PASS. Continue pending TASK-CONV-001 and TASK-AUDIO-001 Windows runtime smoke. Keep committed STT default at `tiny`; `base` and `small` remain override candidates only. |
