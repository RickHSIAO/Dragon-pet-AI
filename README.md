...
**Latest TASK-STT-005 implementation (2026-06-12):** Runtime STT Model Selection UI / base-small Candidate Setting **IMPLEMENTED - AUTOMATED MODEL SELECTION SMOKE PASS / NEEDS WINDOWS RUNTIME MODEL SWITCH SMOKE**. Full App voice settings now include a session-only `STT model` selector (`Default / env`, `tiny`, `base`, `small`). Manual Mic and Conversation Mode send the selected model through the existing `stt:transcribe` bridge only when not using `Default / env`; `/stt/transcribe` accepts optional multipart `model`, validates `tiny/base/small`, and falls back safely on invalid request values. Runtime default remains `tiny`; env overrides remain supported. No `/chat` or mood schema change, Owner Voice hard gate, IPC channel change, queue/pre-roll/drain policy change, raw audio persistence, or committed sample audio change.
