...
**Latest TASK-CONV-003 implementation (2026-06-11):** Conversation Mode Queue Backpressure / Extra Dropped Turn Investigation **IMPLEMENTED - AUTOMATED BACKPRESSURE SMOKE PASS / NEEDS WINDOWS RUNTIME 4-TURN RE-SMOKE**. The TASK-CONV-002 `turn#5 queue_full` row was ambiguous because older overflow lifecycle entries did not copy duration/blob/chunk/audio classification and could render as `durationMs=0 bytes=0`. Queue limit remains `2`; real overflow still shows `reason=queue_full`, now with `audio=usable_audio`, `dropStage=at_queue`, duration, bytes, and chunk count. Zero-byte artifacts now drop before queue as `reason=empty_artifact`, `audio=empty_artifact`, `dropStage=before_queue`; duplicate recorder finalization is prevented without overwriting the first terminal lifecycle status. No STT default, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema, IPC, Pet Window, Output Queue, raw audio persistence, path exposure, transcript exposure, centroid exposure, embedding exposure, queue ordering, graceful Stop/drain, or no-parallel `/chat` behavior change.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Run TASK-CONV-003 Windows actual-audio 4-turn re-smoke, then continue pending TASK-CONV-001 and TASK-AUDIO-001 Windows runtime smoke. TASK-PERSONA-001 is closed; keep debug fallback repair enabled unless a future runtime tone task deliberately replaces it. |
