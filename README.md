...
**Latest TASK-CONV-006 implementation (2026-06-13):** Conversation Mode Backpressure Pause / No Usable Queue-Full Drop **IMPLEMENTED - AUTOMATED BACKPRESSURE SMOKE PASS / NEEDS WINDOWS LONG SESSION RE-SMOKE**. Conversation Mode keeps bounded pending queue max `4`, but now pauses starting new VAD-triggered utterance recorders at `pending=4/4` or high watermark `pending>=3/4` while a turn is active/processing, then resumes at `pending<=2/4` or `drain_complete`. Voice Diagnostics now exposes `conversationBackpressurePaused`, `conversationBackpressureReason`, and `conversationBackpressureResumeReason`. Normal long-session runs should pause/resume instead of repeatedly dropping usable audio as `queue_full`; the hard fallback `queue_full` path remains visible with `audio=usable_audio`, `dropStage=at_queue`. No STT default/model selector, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit was added.

**Latest TASK-CONV-005 implementation (2026-06-12):** Conversation Mode Long Session Stability Smoke **IMPLEMENTED - AUTOMATED LONG SESSION DIAGNOSTICS SMOKE PASS / NEEDS WINDOWS 5-10 MIN CONVERSATION SMOKE**. `apps/desktop/scripts/conversation-long-session-smoke.js` verifies the renderer diagnostics source shape and, as of TASK-CONV-006, a synthetic 13-turn lifecycle fixture for longer Conversation Mode sessions. It guards bounded queue max `4`, queue pressure/full visibility, `drain_complete`, backpressure pause/resume, distinct hard fallback `queue_full` usable-audio overflow vs `empty_artifact` before-queue drops, Owner Voice dry-run visibility, candidate WAV deletion facts, and the copyable long-session summary format. This remains diagnostics/checklist coverage; TASK-CONV-006 adds the runtime pause/resume behavior and still requires Windows long-session re-smoke.
