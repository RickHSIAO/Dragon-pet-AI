...
**Latest TASK-PERSONA-002 implementation (2026-06-12):** Christina General Tone Sanitizer for Non-Debug Replies **IMPLEMENTED - PROMPT/REPAIR SMOKE PASS / NEEDS WINDOWS GENERAL TONE SMOKE**. Added positive non-debug tone guidance so Christina stays proud, teasing, witty, and protective without contempt, hostile unclear-input dismissal, or repeated humiliation. The LLM success path now applies the existing TASK-PERSONA-001 debug repair first, then a narrow general repair for known harsh fragments, unclear-input hostility, repeated humiliation, and likely garbled-STT cases. Safe tsundere lines are preserved. No `/chat` schema, mood schema, STT default, Owner Voice hard gate, renderer IPC, provider runtime, or runtime model change.
