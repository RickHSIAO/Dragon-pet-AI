...
**Latest TASK-CONV-004 implementation (2026-06-11):** Conversation Mode Queue Capacity / Backpressure Policy **IMPLEMENTED - AUTOMATED QUEUE POLICY SMOKE PASS / NEEDS WINDOWS RUNTIME FAST 4-TURN SMOKE**. Windows re-smoke after TASK-CONV-003 proved the missing fourth utterance was real usable audio dropped at `pending=2/2` while `activeTurnId=1`. Conversation Mode now uses a bounded hardcoded pending queue capacity of `4`, still processes STT/chat sequentially with no parallel `/chat`, and adds safe queue pressure diagnostics (`conversationQueuePressure`, `conversationQueueFull`). Real overflow beyond capacity still shows `reason=queue_full`, `audio=usable_audio`, `dropStage=at_queue`, duration, bytes, and chunks; zero-byte artifacts still drop before queue as `reason=empty_artifact`, `audio=empty_artifact`, `dropStage=before_queue`. No STT default, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema, IPC, Pet Window, Output Queue, raw audio persistence, path exposure, transcript exposure, centroid exposure, or embedding exposure change.
...
| Demo 可用（本地 Ollama） | ✅ 是 |
| 下一個任務 | Run TASK-CONV-004 Windows runtime fast 4-turn smoke, then continue pending TASK-CONV-001 and TASK-AUDIO-001 Windows runtime smoke. TASK-PERSONA-001 is closed; keep debug fallback repair enabled unless a future runtime tone task deliberately replaces it. |
