...
| TASK-PERSONA-001 | Christina tsundere tone boundary | Tunes the runtime persona prompt to keep proud / tsundere / protective Christina flavor while discouraging repeated direct humiliation, hostile name-calling, repeated address labels, and dismissive technical-debug responses. Fourth-pass tuning removes literal negative examples from LLM-visible prompt content, adds user-message-aware debug-intent prompt guidance, and applies a narrow post-generation repair for known harsh/evasive debug templates. Windows runtime tone smoke PASS confirmed useful STT/debug, Conversation Mode, broad issue, closeout, and tired/stress replies. No `/chat` schema, mood schema, STT, Owner Voice, Ollama provider, renderer UI, IPC, or runtime model change. DONE - Windows chat tone smoke PASS; debug fallback repair enabled. |
| TASK-PERSONA-002 | Christina general tone sanitizer | Adds positive non-debug prompt guidance for proud, teasing, witty, protective replies without contempt, hostile unclear-input dismissal, comparative devaluation, threat/intimidation phrasing, waste-time hostility, or refusal/abandoning phrasing for companion/testing requests. Repair order is fixed: TASK-PERSONA-001 debug repair first, TASK-PERSONA-002 general repair second. General repair handles known harsh fragments, repeated humiliation, unclear-input hostility, likely garbled-STT cases, comparative devaluation, threats, waste-time hostility, and companion/testing refusal fragments. Windows general tone smoke passed with cooperative companion/testing replies, acceptable light sass, garbled-STT handling, and STT/debug `NEEDS EVIDENCE`. No `/chat` schema, mood schema, STT default, Owner Voice hard gate, renderer IPC, provider runtime, or runtime model change. DONE - Windows general tone smoke PASS; general repair enabled. |
...
| TASK-SEC-002 | Sensitive data inventory / redaction rules | Defines S0-S6 classes, exposure matrix, redaction patterns, Owner Voice Gate API/diagnostics/logging restrictions, and TASK-SEC-003 corpus plan. DOCS ONLY. |
...
| TASK-STT-002 | Chinese STT quality baseline | Adds `scripts/stt_quality_probe.py` for local faster-whisper `tiny` / `base` / `small` / optional `medium` comparison on explicit WAV/reference samples or a private manifest. Reports raw/corrected/punctuated/final transcripts, character error rate, latency, audio duration, real-time factor, repeated-token and empty warnings, and sourceType grouping for capture-vs-model diagnosis. Missing models skip by default unless `--allow-download` is passed. No runtime default model change, Conversation Mode queue change, Owner Voice Gate behavior change, schema change, IPC, raw audio persistence, or aggressive rewrite. |
| TASK-STT-003 | Runtime STT model override | Adds `DRAGON_STT_MODEL` as the preferred short runtime override for Manual Mic and Conversation Mode candidate smoke while preserving legacy `DRAGON_PET_STT_MODEL` and default `tiny`. `base` is the first runtime candidate from TASK-STT-002; `small` is a slower quality candidate. Diagnostics show requested/resolved model, source, env, fallback reason, load status/error, provider state, and final transcript path. No default switch, queue/pre-roll/drain behavior change, Owner Voice Gate change, schema break, IPC, raw audio persistence, or aggressive rewrite. |
| TASK-STT-004 | No-speech / silence hallucination guard | Adds conservative WAV PCM audio-energy diagnostics and faster-whisper segment metadata checks before accepting silent/near-silent STT output. Windows runtime no-speech smoke PASS for Manual Mic silence and real speech with `DRAGON_STT_MODEL=small` and `DRAGON_STT_MODEL=base`, plus Conversation Mode silence / no-speech path. Guarded results return `status=no_speech`, empty final transcript, and safe diagnostics; Manual Mic does not fill textarea or auto-send, and Conversation Mode does not enqueue/send chat while preserving rearm and graceful drain. Suspicious subtitle-credit and creator-CTA phrases are suppressed only with weak/no-speech evidence; real speech is not suppressed. Runtime default remains `tiny`; `base` and `small` remain override candidates only. No default model switch, `/chat` schema change, Owner Voice hard gate, raw audio persistence, private path exposure, or aggressive rewrite. |
| TASK-STT-005 | Runtime STT model selection UI | Adds a session-only Full App `STT model` selector with `Default / env`, `tiny`, `base`, and `small`. `/stt/transcribe` accepts an optional multipart `model` field; empty/omitted keeps env/default resolution, valid request values use `modelSource=request`, and invalid request values fall back safely with `invalid_request_model`. Manual Mic and Conversation Mode use the existing `stt:transcribe` IPC bridge; `Default / env` sends no request model, while explicit `tiny`/`base`/`small` sends a request model. Diagnostics show UI selected model, request model sent, requested/resolved model, request source, fallback reason, provider load state, and model load error. Runtime default remains `tiny`; Windows runtime model-switch smoke PASS for Manual Mic, Manual Auto-send, and Conversation Mode; `base` and `small` remain runtime candidates only. No `/chat` or mood schema change, Owner Voice hard gate, IPC channel change, queue/pre-roll/drain change, or raw audio persistence. |
| TASK-STT-006A | Backend STT model evaluation report | Adds `scripts/stt_model_evaluation_report.py` to collect backend-side comparison data for `tiny`, `base`, and `small` on the same local audio samples. The script calls the existing runtime STT service path with per-request model selection, writes JSON reports under `outputs/stt_model_evaluation/YYYYMMDD/`, and records redacted sample basenames, duration/file size, latency/RTF, raw/final transcripts, no-speech guard diagnostics, provider/model load status, fallback, and per-model errors. Windows local WAV evaluation PASS confirmed report data collection is ready: all three models completed 2/2 owner samples successfully with no `no_speech` or error result. Full report flow is backend evaluation report -> deterministic scoring -> grounded explanation; this task implements only the report stage. No recommended model, subjective AI explanation, frontend comparison panel, STT default change, `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed audio sample, or generated report. |
| TASK-STT-006B | Deterministic STT model scoring | Adds `scripts/stt_model_scoring_report.py` to read a TASK-STT-006A JSON report and produce deterministic runtime-suitability scores under `outputs/stt_model_scoring/YYYYMMDD/`. Profiles `manual_mic`, `conversation`, and `balanced` weight reliability, speed, speech evidence, transcript signal, hallucination risk, and fallback penalties; `conversation` penalizes slow models harder. Windows scoring report smoke PASS for balanced, conversation, and manual_mic profiles. The flow is now evaluation report -> deterministic scoring report -> grounded explanation. This task implements only deterministic scoring and does not claim true transcript accuracy/WER without reference transcripts. No LLM/AI explanation, frontend comparison panel, runtime auto-switch, STT default change, `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed audio, or generated report. |
| TASK-STT-006C | Grounded STT recommendation explanation | Adds `scripts/stt_model_recommendation_explanation.py` to read a TASK-STT-006B scoring report and write grounded explanation JSON under `outputs/stt_model_explanation/YYYYMMDD/`. Styles `christina` and `plain` use only scoring-report facts and include caveats for runtime-suitability-only scoring, no reference transcript/WER evidence, small sample limits, no default change, and no runtime auto-switch. Windows grounded explanation smoke PASS and Christina zh-TW localization PASS. The final report pipeline is complete: 006A evaluation report -> 006B deterministic scoring -> 006C grounded explanation. Runtime STT selection remains manual through TASK-STT-005; no frontend comparison panel, runtime auto-switch, STT default change, `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed audio, or generated report. |
| TASK-STT-007 | STT model advisor runner | Adds `scripts/stt_model_advisor_runner.py` as a CLI orchestration layer over 006A/006B/006C. It runs evaluation -> deterministic scoring -> grounded explanation in one command, writes normal intermediate runtime artifacts, and writes a final advisor manifest under `outputs/stt_model_advisor/YYYYMMDD/` with stage basenames, recommendation facts, explanation text, and safety flags. Windows end-to-end runner smoke PASS confirmed the one-command advisor workflow with two local owner WAV samples. Runtime STT selection remains manual through TASK-STT-005; no frontend comparison panel, runtime auto-switch, STT default change, `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed audio, or generated report. DONE - Windows end-to-end advisor runner smoke PASS / one-command STT advisor ready. |
| TASK-CONV-001 | Conversation Mode continuous capture queue | Separates Conversation Mode capture state from processing state and adds a bounded in-memory pending utterance queue. As of TASK-CONV-004 the pending queue capacity is `max=4`. Listening can resume after an utterance WAV Blob is finalized while prior STT and `/chat` processing continues. Queue processing stays ordered and sequential with no simultaneous multiple `/chat` requests. Overflow drops newest with sanitized diagnostics. Stop is graceful drain: new capture stops immediately, already-recorded turns are preserved and drained, and active recording Stop finalizes the last turn when valid. No TTS interruption/barge-in, schema change, IPC, Pet Window, Output Queue, Owner Voice hard gate, raw audio persistence, path exposure, centroid exposure, or embedding exposure. IMPLEMENTED - automated renderer smoke PASS; needs Windows runtime smoke. |
| TASK-CONV-002 | Conversation Mode turn lifecycle diagnostics | Adds bounded session-only recent turn lifecycle diagnostics for missing-turn investigation. Voice Diagnostics shows safe `turn#N` lifecycle rows covering recorded/finalized/queued/dropped/STT/chat/drain states, queue action/reason, pending/active counters, Stop/drain state, duration/blob/pre-roll facts, STT status, chat status, and Owner Voice dry-run status. Windows runtime 4-turn smoke PASS confirmed #1-#4 completed and sent, dropped-turn visibility via `turn#5 queue_full`, and drain visibility via `turn#6 drain_complete`. No STT default, Owner Voice hard gate, schema, IPC, Pet Window, Output Queue, raw audio, path, transcript, centroid, or embedding exposure change. DONE - Windows runtime 4-turn lifecycle smoke PASS. |
| TASK-CONV-003 | Conversation Mode queue backpressure artifacts | Adds safe lifecycle classification for queue backpressure and duplicate finalization investigation. `queue_full` rows now include duration/blob/chunk facts and `audio=usable_audio`, `dropStage=at_queue`; `0B` artifacts drop before queue admission as `reason=empty_artifact`, `audio=empty_artifact`, `dropStage=before_queue`; duplicate recorder finalization is ignored without overwriting the first terminal lifecycle status. Windows re-smoke identified a real usable-audio capacity drop at `pending=2/2`; TASK-CONV-004 addressed the follow-up by raising bounded pending capacity to `4` and passing Windows runtime fast 4-turn smoke. No STT default, Owner Voice hard gate, schema, IPC, Pet Window, Output Queue, raw audio persistence, path/transcript/centroid/embedding exposure, queue ordering, graceful Stop/drain, or no-parallel `/chat` behavior change. DONE - automated backpressure smoke PASS / follow-up addressed by TASK-CONV-004. |
| TASK-CONV-004 | Conversation Mode queue capacity/backpressure policy | Raises the hardcoded bounded pending queue capacity from `2` to `4` after Windows runtime evidence showed a normal fourth utterance was real usable audio dropped at capacity. Adds `conversationQueuePressure` and `conversationQueueFull` diagnostics. Four pending turns can be accepted while one active turn is processing; overflow beyond four still drops newest as visible `queue_full` with `audio=usable_audio`, `dropStage=at_queue`, duration, bytes, and chunks. Empty artifacts still drop before queue as `empty_artifact` before queue admission. STT/chat processing remains ordered and one-at-a-time; graceful Stop/drain remains unchanged. Windows runtime fast 4-turn actual-audio smoke with `DRAGON_STT_MODEL=base` PASS: turns #1-#4 accepted/sent, no usable-audio `queue_full`, final pending `0/4`, queue pressure `empty full=false`, final off/idle/active `0`/drain_complete, Owner Voice dry-run non-blocking and temporary WAV deleted. No STT default, Owner Voice hard gate, schema, IPC, Pet Window, Output Queue, raw audio/path/transcript/centroid/embedding exposure change. DONE - Windows runtime fast 4-turn smoke PASS. |
| TASK-CONV-005 | Conversation Mode long-session stability smoke | Adds an automated diagnostics source-shape and synthetic lifecycle smoke for longer Conversation Mode runs plus a Windows 5-10 minute manual checklist. The guard verifies queue max remains `4`, queue pressure/full and `drain_complete` remain visible, `queue_full` usable-audio overflow stays distinct from `empty_artifact` before-queue drops, lifecycle rows retain audio/drop/Owner Voice/candidate WAV cleanup fields, and Owner Voice dry-run remains non-blocking. The runtime selector remains the manual TASK-STT-005 selector; no STT default, runtime model selector behavior, queue policy, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit changed. DONE - addressed / validated by TASK-CONV-006 Windows long-session re-smoke PASS. |
| TASK-CONV-006 | Conversation Mode backpressure pause/resume | Keeps pending queue max at `4` and adds a pause/resume throttle before new VAD-triggered utterance recorders start. Capture pauses at `pending=4/4`, or at high watermark `pending>=3/4` while a turn is active/processing, and resumes at `pending<=2/4` or `drain_complete`. Diagnostics expose `conversationBackpressurePaused`, `conversationBackpressureReason`, and `conversationBackpressureResumeReason`. Windows long-session re-smoke validated this path with `queue_high_watermark` pause at `pending=3/4 active=10`, final `pending=0/4`, `activeTurnId=0`, `queue=idle/drain_complete`, `stopMode=drain_complete`, and final backpressure `paused=false reason=none resume=queue_available`. Normal long sessions now pause capture instead of repeatedly producing usable-audio `queue_full`; `queue_full` remains a visible hard fallback with `audio=usable_audio`, `dropStage=at_queue`, not the expected normal long-session path. Empty artifacts remain distinct. No STT default, STT model selector, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit changed. DONE - Windows long-session backpressure re-smoke PASS / no repeated usable queue_full drops. |
| TASK-TTS-001 | Local TTS provider architecture | Adds docs-only provider-neutral TTS architecture and provider research. Target future pipeline is accepted chat reply -> text normalization -> TTS queue -> provider adapter -> playback -> Pet speaking state. TTS remains a post-reply output layer, starts with mock provider tests, prefers local/offline provider experiments, and does not lock to ElevenLabs or any paid external API. Conversation Mode STT/backpressure remains independent: queue max stays `4`, `queue_full` remains hard fallback, and future TTS must not block drain or feed audio into the microphone. No runtime TTS provider, dependency, generated audio, voice sample, `/chat` schema, mood schema, STT default/model selector, Owner Voice hard gate, IPC, Pet Window runtime, or auto-speaking change. DONE - TTS architecture design ready / implementation not started. |
| TASK-TTS-002 | Mock TTS provider skeleton | Adds backend metadata-only `backend/app/tts/` skeleton: `TTSProvider` protocol, deterministic `MockTTSProvider`, conservative text normalization/chunking, `TTSService`, and disabled queue diagnostics. Defaults remain `TTS_ENABLED=false`, provider `mock`, voice `christina_mock`, `queueLength=0`, `activeJobId=null`, `autoSpeakEnabled=false`, and playback `disabled` / `not_started`. No `/tts` route, renderer controls, Pet Window runtime, real synthesis, real playback, generated audio, dependency, ElevenLabs/cloud integration, `/chat` schema, mood schema, STT default/model selector, Conversation Mode queue/backpressure, Owner Voice hard gate, or auto-speaking change. IMPLEMENTED - mock TTS skeleton smoke PASS / runtime playback not started. |
| TASK-AUDIO-001 | Capture latency + Conversation pre-roll | Adds safe per-capture timing diagnostics and a bounded 500 ms in-memory PCM pre-roll buffer for Conversation Mode. Manual Mic shows preparing until recorder start succeeds. Conversation pre-roll is prepended on VAD trigger and cleared after use/Stop. No raw audio persistence, pre-roll content exposure, STT model change, transcript rewrite, schema change, IPC, Pet Window, Output Queue, or Owner Voice hard gate. IMPLEMENTED - automated renderer smoke PASS; needs Windows runtime smoke. |
...
Future TASK-TTS-001 voice-output architecture is separate from the current
interaction mirror stack:
...
TASK-TTS-002 implements only the backend mock provider, normalizer, and disabled
queue diagnostics for this pipeline. Playback, route/UI controls, Pet speaking
state, and real provider synthesis remain future. TTS must remain a post-reply
output layer: it cannot call `/chat`, cannot read hidden diagnostics, cannot
write chat history, and cannot alter Conversation Mode STT queue, backpressure,
or drain behavior. Conversation Mode queue max remains `4`, and `queue_full`
remains a hard fallback diagnostics path.
...
| TTS skeleton | Accepted assistant reply text for explicit backend preview/test calls | TTS-safe chunks, mock provider metadata, disabled queue diagnostics | Backend service only | TASK-TTS-002; starts disabled; no `/chat`; no route/UI controls; no hidden diagnostics; no Conversation Mode queue change; no playback. |
...
direct observation, needed evidence, and a next check; broad debug questions
should use PASS / FAIL / NEEDS EVIDENCE framing when evidence is insufficient.
Fourth-pass tuning removes literal negative examples from the LLM-visible prompt,
adds optional debug-intent prompt guidance based on the current user message, and
repairs only known harsh/evasive generated templates. This does not change
`/chat` response shape, mood schema, renderer UI, IPC, STT, Owner Voice, Ollama
provider behavior, or runtime model defaults. Windows runtime tone smoke passed
after fourth-pass hardening; debug contexts should continue to prioritize direct
usefulness before teasing.

TASK-PERSONA-002 adds the second runtime prompt/repair layer for non-debug
general replies. It keeps the same positive-only prompt principle and tells
Christina to clarify unclear input without blaming the user, mention likely STT
recognition errors when input appears garbled, and avoid contempt, dominating
framing, repeated humiliation, comparative devaluation, threats, intimidation,
waste-time hostility, and refusal/abandoning phrasing for companion/testing
requests while preserving proud tsundere flavor. The post-generation repair
remains narrow: TASK-PERSONA-001 debug repair runs first, then the general
repair only rewrites known harsh fragments, unclear-input hostility, repeated
humiliation, likely garbled-STT hostility, comparative devaluation,
threat/intimidation fragments, waste-time hostility, or companion/testing
refusal fragments. Windows general tone smoke passed with this repair order:
debug fallback first, general repair second. This does not change `/chat`
response shape, mood schema, renderer IPC, STT default, Owner Voice hard-gate
behavior, provider runtime, or runtime model defaults.
...
- TASK-STT-002 IMPLEMENTED - CHINESE STT QUALITY BASELINE / MODEL COMPARISON (2026-06-11):
  `scripts/stt_quality_probe.py` provides a repeatable local benchmark for Chinese STT quality using
  explicit WAV/reference pairs or a private manifest. It compares faster-whisper `tiny`, `base`,
  `small`, and optional `medium`; missing models are skipped by default unless `--allow-download`
  is passed. The report keeps bad raw output visible and includes raw/corrected/punctuated/final
  transcripts, reference, character error rate, raw character error rate, latency, audio duration,
  real-time factor, repeated-token warnings, empty-transcript warnings, and error text. `sourceType`
  grouping distinguishes `manual_mic`, `conversation_mode`, and `external_clean_reference` samples
  for capture-vs-model diagnosis. Runtime still defaults to faster-whisper `tiny` via
  `DRAGON_PET_STT_MODEL`; this task does not change runtime defaults, Conversation Mode queue
  behavior, Owner Voice Gate behavior, `/stt/transcribe` or `/chat` schemas, IPC, raw audio
  persistence, or transcript rewrite policy.
- TASK-STT-003 IMPLEMENTED - RUNTIME STT MODEL OVERRIDE / BASE-SMALL CANDIDATE SMOKE (2026-06-11):
  Backend model resolution now accepts `DRAGON_STT_MODEL` as the preferred short env override,
  then legacy `DRAGON_PET_STT_MODEL`, then default `tiny`. Allowed values remain `tiny`, `base`,
  and `small`; invalid values fall back to `tiny` with `modelFallbackReason=invalid_model`.
  `/stt/transcribe` diagnostics include `requestedModel`, `resolvedModel`, `modelSource`,
  `modelEnv`, `modelFallbackReason`, `modelLoadStatus`, `modelLoadError`, provider
  source/load/fallback fields, and the existing raw/corrected/punctuated/final transcript fields.
  Manual Mic and Conversation Mode receive the same final transcript path as before. `base` is the
  first runtime candidate from TASK-STT-002; `small` remains a slower quality candidate. No default
  switch, Conversation Mode queue change, TASK-AUDIO-001 pre-roll change, TASK-CONV-001 graceful
  drain change, Owner Voice Gate change, schema break, IPC, raw audio persistence, or aggressive
  rewrite was added.
- TASK-STT-005 DONE - WINDOWS RUNTIME MODEL SWITCH SMOKE PASS / DEFAULT UNCHANGED (2026-06-12):
  Full App voice settings now include a session-only `STT model` selector:
  `Default / env`, `tiny`, `base`, and `small`. Manual Mic and Conversation
  Mode both call the existing `stt:transcribe` bridge; the renderer sends a
  multipart `model` field to `/stt/transcribe` only for explicit
  `tiny`/`base`/`small` selections. `Default / env` sends no model field and
  preserves the existing `DRAGON_STT_MODEL` -> `DRAGON_PET_STT_MODEL` ->
  committed default `tiny` resolution. Backend request values resolve with
  `modelSource=request`; invalid request values fall back without crashing and
  surface `modelFallbackReason=invalid_request_model`. Diagnostics distinguish
  UI selected model, request model sent, requested/resolved model, source,
  fallback reason, provider load state, and model load error. Windows runtime
  smoke passed for `base` Manual Mic, `small` Manual Auto-send, `Default / env`
  Manual Auto-send, and `base` Conversation Mode. Conversation Mode finished
  with `stt=success`, `chat=sent`, final `pending=0/4 active=0`, and
  `stopMode=drain_complete`; Owner Voice dry-run stayed non-blocking and
  candidate WAV cleanup succeeded. No `/chat` or mood schema change, Owner
  Voice hard gate, IPC channel change, Conversation queue/pre-roll/drain policy
  change, raw audio persistence, or default model switch was added.
- TASK-STT-006A DONE - WINDOWS AUDIO SAMPLE EVALUATION PASS / REPORT DATA COLLECTION READY (2026-06-12):
  Backend STT model evaluation now starts with a script/report stage:
  `scripts/stt_model_evaluation_report.py` compares `tiny`, `base`, and
  `small` on the same local user-provided samples by calling the runtime STT
  service with per-request model selection. Reports write to
  `outputs/stt_model_evaluation/YYYYMMDD/` and include safe environment info,
  redacted sample basenames, duration/file size, latency/RTF, raw/final
  transcript fields, no-speech guard diagnostics, audio signal metrics,
  provider/model load state, fallback, and per-model errors. The intended
  future chain is backend evaluation report -> deterministic scoring -> AI
  explanation; this task implements only the report stage. No recommendation,
  subjective AI explanation, frontend comparison panel, default model switch,
  `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed
  audio sample, or generated report was added. Windows local WAV evaluation
  passed for `tiny`, `base`, and `small` on two owner samples: all three models
  completed 2/2 samples successfully with no `no_speech` or error result.
- TASK-STT-006B DONE - WINDOWS SCORING REPORT SMOKE PASS / DETERMINISTIC SCORING READY (2026-06-12):
  Deterministic STT model selection now has a scoring-report stage after 006A:
  `scripts/stt_model_scoring_report.py` reads an evaluation JSON, computes
  model aggregates and 0-100 runtime-suitability scores, and writes reports
  under `outputs/stt_model_scoring/YYYYMMDD/`. The completed scoring stage feeds
  the grounded explanation stage added by TASK-STT-006C. Profiles are
  `manual_mic`, `conversation`, and `balanced`; the conversation profile weights
  speed more heavily. Windows scoring smoke PASS: all three profiles generated
  scoring JSON, aggregates, model scores, and deterministic recommendations for
  `tiny`, `base`, and `small`; `base` ranked highest in this local two-sample
  runtime-suitability smoke. No true transcript accuracy/WER is claimed without
  reference transcripts. TASK-STT-006B does not add LLM/AI explanation,
  frontend comparison panel, runtime auto-switch, default model change, `/chat`
  or mood schema change, Owner Voice hard gate, renderer IPC, committed audio,
  or generated report.
- TASK-STT-006C DONE - WINDOWS GROUNDED EXPLANATION SMOKE PASS / CHRISTINA ZH-TW EXPLANATION READY (2026-06-12):
  Adds a grounded explanation stage after deterministic scoring:
  `scripts/stt_model_recommendation_explanation.py` reads a TASK-STT-006B
  scoring report and writes explanation JSON under
  `outputs/stt_model_explanation/YYYYMMDD/`. The final report pipeline is
  complete: 006A evaluation report -> 006B deterministic scoring -> 006C
  grounded explanation. `christina` and `plain` styles both use only
  scoring-report facts and include uncertainty/caveats; Christina output is
  localized to Traditional Chinese proud/tsundere helpful wording. The current
  implementation is deterministic/no-LLM by default; `--use-llm` remains a
  future-compatible fallback path and does not call a live provider. Runtime
  selection remains manual through the existing TASK-STT-005 selector. No
  frontend comparison panel, runtime auto-switch, default model change, `/chat`
  or mood schema change, Owner Voice hard gate, renderer IPC, committed audio,
  or generated report was added.
- TASK-STT-007 DONE - WINDOWS END-TO-END ADVISOR RUNNER SMOKE PASS / ONE-COMMAND STT ADVISOR READY (2026-06-12):
  Adds `scripts/stt_model_advisor_runner.py` as a CLI orchestration layer over
  the completed report stages: 006A evaluation report -> 006B deterministic
  scoring -> 006C grounded explanation. It reuses the existing stage functions,
  writes normal intermediate runtime artifacts, and writes a final advisor
  manifest under `outputs/stt_model_advisor/YYYYMMDD/` with stage report
  basenames, deterministic recommendation facts, explanation text, and safety
  flags. Windows end-to-end runner smoke PASS completed the one-command workflow
  on two local owner WAV samples and produced evaluation, scoring, explanation,
  and advisor reports. Runtime model selection remains manual through the existing
  TASK-STT-005 selector. No frontend comparison panel, runtime auto-switch,
  default model change, `/chat` or mood schema change, Owner Voice hard gate,
  renderer IPC, committed audio, or generated report was added.
- TASK-CONV-001 IMPLEMENTED - CONVERSATION MODE CONTINUOUS CAPTURE QUEUE (2026-06-05):
  Conversation Mode now separates capture state (`off/listening/recording`) from processing state
  (`idle/stt_processing/chat_processing`) and tracks queue state with `pendingCount`,
  `activeTurnId`, and last queue action/reason diagnostics. After the recorder finalizes a WAV Blob,
  capture can return to listening while the queued utterance continues through STT and `/chat`.
  As of TASK-CONV-004, the queue is bounded to 4 pending utterances and drops newest on overflow with `queue_full`.
  Processing remains a single ordered worker, so there are no simultaneous multiple `/chat`
  requests. Stop is graceful drain: it stops VAD/listening and mic capture immediately,
  preserves already-recorded pending turns, finalizes an active recorder as the last queued turn
  when valid, drains in order, and finishes at capture `off` / processing `idle` / pending `0`.
  Utterances beginning after Stop are dropped. A separate future Force Stop / Cancel Pending
  action may clear queued turns; normal Stop does not. STT failure on one queued item does not
  deadlock later items. This does not add TTS interruption/barge-in,
  full-duplex chat, new IPC, `/stt/transcribe` schema changes, `/chat` schema changes, Pet Window
  changes, Output Queue changes, Owner Voice hard gate behavior, raw audio persistence, candidate
  path exposure, centroid exposure, or embedding exposure. Automated renderer smoke PASS; Windows
  runtime smoke with actual audio is still required before DONE. User-facing DONE acceptance
  remains blocked by observed capture-start clipping and STT recognition quality until
  TASK-AUDIO-001 Windows capture/pre-roll smoke and a later STT quality benchmark both pass.
- TASK-CONV-002 DONE - CONVERSATION MODE TURN LIFECYCLE DIAGNOSTICS (2026-06-11):
  Conversation Mode now records a bounded session-only recent turn lifecycle history for
  missing-turn diagnosis. Each entry is keyed by `turnId` and records only safe scalar facts:
  capture source, lifecycle status/reason, queue action/reason, pending and active turn counts,
  Stop/drain flags and mode, duration, blob size, chunk count, mime type, pre-roll milliseconds,
  STT status, chat status, and Owner Voice dry-run status. Voice Diagnostics renders compact
  `turn#N lifecycle ...` rows via `textContent`. Dropped turns, no-speech turns, STT errors,
  chat errors, skipped sends, successful chat sends, and graceful drain completion remain visible
  without exposing transcript text, candidate paths, raw audio, centroid data, embeddings, or
  local settings. This does not change TASK-CONV-001 queue ordering, max pending now `4` as of TASK-CONV-004, no-parallel
  `/chat`, or graceful Stop/drain behavior, and it does not change STT defaults, Owner Voice hard
  gate behavior, schemas, IPC, Pet Window, or Output Queue. Automated diagnostics smoke PASS and
  Windows runtime 4-turn smoke PASS: `turn#1` through `turn#4` completed with `stt=success` and
  `chat=sent`; durations were non-negative; valid turns had non-zero bytes; Owner Voice dry-run
  remained non-blocking with candidate WAV temporary/deleted; final state was capture `off`,
  processing `idle`, pending `0/2`, `activeTurnId=0`, and `stopMode=drain_complete`. The same
  smoke intentionally records the visible `turn#5 dropped reason=queue_full` and `turn#6
  drain_complete` evidence. Track the extra `queue_full` drop as TASK-CONV-003 - Conversation Mode
  Queue Backpressure / Extra Dropped Turn Investigation.
- TASK-CONV-003 DONE - CONVERSATION MODE QUEUE BACKPRESSURE ARTIFACTS / FOLLOW-UP ADDRESSED BY TASK-CONV-004 (2026-06-12):
  The renderer now records safe classification fields on lifecycle entries so
  backpressure can be separated from empty or duplicate recorder artifacts.
  The TASK-CONV-002 runtime `turn#5 queue_full` row was ambiguous because the
  older `queue_full` branch did not copy candidate duration, Blob bytes, chunk
  count, or audio class into the lifecycle entry; that could render overflow as
  `durationMs=0 bytes=0`. TASK-CONV-003 kept the existing capacity and keeps real
  overflow visible as `reason=queue_full`, now with `audio=usable_audio`,
  `dropStage=at_queue`, duration, bytes, and chunk count. Zero-byte candidates
  are dropped before queue admission as `reason=empty_artifact`,
  `audio=empty_artifact`, and `dropStage=before_queue`. Duplicate recorder
  finalization callbacks set `duplicateFinalizePrevented=true` and update the
  attempt count without overwriting the first terminal lifecycle status. Voice
  Diagnostics remains text-only and session-only, and it still exposes only safe
  scalar facts. No STT default, Owner Voice hard gate, schema, IPC, Pet Window,
  Output Queue, raw audio persistence, candidate path exposure, transcript
  exposure, centroid exposure, embedding exposure, queue ordering, graceful
  Stop/drain, or no-parallel `/chat` behavior changed. Renderer backpressure
  smoke PASS. Windows actual-audio re-smoke with `DRAGON_STT_MODEL=base`
  confirmed the missing fourth utterance was real usable audio dropped at
  `pending=2/2`, `activeTurnId=1`, `durationMs=3098`, `bytes=106540`,
  `audio=usable_audio`, and `dropStage=at_queue`; TASK-CONV-004 handled that
  capacity policy by raising bounded capacity to `4` and passing the Windows
  runtime fast 4-turn smoke.
- TASK-CONV-004 DONE - CONVERSATION MODE QUEUE CAPACITY / BACKPRESSURE POLICY (2026-06-12):
  Conversation Mode pending queue capacity is now a hardcoded bounded default of
  `FULL_APP_CONVERSATION_PENDING_MAX = 4`. This follows Windows runtime evidence
  that a normal fourth utterance could be dropped as real usable-audio overflow
  at the old capacity while an active turn was still processing. The queue
  remains finite and the worker remains sequential: STT and `/chat` still process
  one turn at a time in capture order, with no parallel `/chat` requests. Voice
  Diagnostics now includes `conversationQueuePressure` (`empty`, `normal`,
  `high`, `full`) and `conversationQueueFull` alongside capacity, pending count,
  active turn ID, and last queue action/reason. Four pending turns can be
  accepted while one active turn is processing. Overflow beyond four pending
  turns still drops newest as `reason=queue_full`, `audio=usable_audio`,
  ...
  `dropStage=before_queue`. Graceful Stop/drain behavior remains unchanged. No
  STT default, Owner Voice hard gate, `/stt/transcribe` schema, `/chat` schema,
  IPC, Pet Window, Output Queue, raw audio persistence, path exposure,
  transcript exposure, centroid exposure, or embedding exposure changed.
  Renderer queue policy smoke PASS. Windows fast 4-turn actual-audio smoke with
  `DRAGON_STT_MODEL=base` PASS: turns #1-#4 were accepted and sent with
  `stt=success`, `chat=sent`, `audio=usable_audio`, and `dropStage=none`;
  turn #4 reached `drain_complete` with `bytes=81964` and `preRollMs=256`;
  diagnostics showed queue capacity `4`, final `pending=0/4`, pressure `empty
  full=false`, conversation/capture `off`, processing `idle`, `activeTurnId=0`,
  queue action `idle`, queue reason `drain_complete`, and stop mode
  `drain_complete`. Owner Voice dry-run stayed non-blocking with
  `accepted=true`, `runtimeHardBlocked=false`, `candidateWavTemporary=true`,
  and `candidateWavDeleted=true`.
- TASK-AUDIO-001 IMPLEMENTED - CAPTURE START LATENCY MEASUREMENT / CONVERSATION PRE-ROLL BUFFER (2026-06-05):
  Voice Diagnostics now records safe per-capture timing fields: `captureRequestedAt`,
  `mediaStreamReadyAt`, `recorderStartRequestedAt`, `recorderStartedAt`, `firstChunkAt`,
  `vadTriggeredAt`, `recordingFinalizedAt`, `captureReadyLatencyMs`, and
  `firstChunkLatencyMs`. Manual Mic enters a short preparing state and only claims recording
  after `MediaRecorder.start()` succeeds. Conversation Mode keeps a bounded 500 ms in-memory
  PCM pre-roll buffer while listening, prepends it when VAD triggers, records
  `preRollAppliedMs`, and clears stale pre-roll after use/Stop. No raw audio persistence,
  pre-roll content/path exposure, STT model change, transcript rewrite, Owner Voice hard gate,
  schema change, IPC, Pet Window, or Output Queue change. Renderer smoke PASS; Windows playback
  smoke with actual audio remains required.
...
- TASK-265 DONE - Windows backend verify-files smoke PASS (2026-06-04):
  ...
  `{ "paths": [str, ...], "threshold": float (optional) }`. Validates request
  fields (rejects rawAudio/base64Audio/transcript/embeddingAggregate), checks
  enrollment state, validates WAV path existence, then calls
  `run_owner_voice_verification_sidecar()` which invokes
  ...
  `sampleCount`, `checkedAudioFiles`, and hardcoded privacy flags all false;
  stored centroid vector never appears in response. 7 new pytest tests; smoke
  section `[14/14]` added. No mic, no audio bytes, no IPC, no STT, no `/chat`,
  no Pet Window, no Output Queue runtime change.
- Relationship state.
