...
| TASK-PERSONA-002 | Christina general tone sanitizer | Adds positive non-debug prompt guidance for proud, teasing, witty, protective replies without contempt, hostile unclear-input dismissal, comparative devaluation, threat/intimidation phrasing, or waste-time hostility. Keeps TASK-PERSONA-001 debug repair first, then applies narrow general repair for known harsh fragments, repeated humiliation, unclear-input hostility, likely garbled-STT cases, comparative devaluation, threats, and waste-time hostility. Safe tsundere lines pass through. No `/chat` schema, mood schema, STT default, Owner Voice hard gate, renderer IPC, provider runtime, or runtime model change. IMPLEMENTED - second prompt/repair smoke PASS; needs Windows general tone re-smoke. |
...
and waste-time hostility while preserving proud tsundere flavor. The
post-generation repair remains narrow: TASK-PERSONA-001 debug repair runs first,
then the general repair only rewrites known harsh fragments, unclear-input
hostility, repeated humiliation, likely garbled-STT hostility, comparative
devaluation, threat/intimidation fragments, or waste-time hostility. This does
not change `/chat` response shape, mood schema, renderer IPC, STT default, Owner
Voice hard-gate behavior, provider runtime, or runtime model defaults.
