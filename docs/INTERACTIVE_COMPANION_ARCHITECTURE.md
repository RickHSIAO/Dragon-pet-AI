...
| TASK-STT-006A | Backend STT model evaluation report | Adds `scripts/stt_model_evaluation_report.py` to collect backend-side comparison data for `tiny`, `base`, and `small` on the same local audio samples. The script calls the existing runtime STT service path with per-request model selection, writes JSON reports under `outputs/stt_model_evaluation/YYYYMMDD/`, and records redacted sample basenames, duration/file size, latency/RTF, raw/final transcripts, no-speech guard diagnostics, provider/model load status, fallback, and per-model errors. Future flow is report data -> deterministic scoring -> AI explanation; this task implements only the report stage. No recommended model, subjective AI explanation, frontend comparison panel, STT default change, `/chat` or mood schema change, Owner Voice hard gate, renderer IPC, committed audio sample, or generated report. |
...
- TASK-STT-006A IMPLEMENTED - AUTOMATED REPORT SMOKE PASS / NEEDS WINDOWS AUDIO SAMPLE EVALUATION (2026-06-12):
  ...
  audio sample, or generated report was added.
