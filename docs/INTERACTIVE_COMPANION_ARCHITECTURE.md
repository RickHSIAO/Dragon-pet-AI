...
This TTS pipeline is not implemented by TASK-TTS-001. It must remain a
post-reply output layer: it cannot call `/chat`, cannot read hidden diagnostics,
cannot write chat history, and cannot alter Conversation Mode STT queue,
backpressure, or drain behavior. Conversation Mode queue max remains `4`, and
`queue_full` remains a hard fallback diagnostics path.
...
| Future TTS architecture | Accepted assistant reply text | TTS-safe chunks, queue job, provider result, playback state | Not implemented by TASK-TTS-001 | Starts disabled; no `/chat`; no hidden diagnostics; no Conversation Mode queue change. |
