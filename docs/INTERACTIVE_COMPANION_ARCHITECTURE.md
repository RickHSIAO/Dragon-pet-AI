...
| TASK-CONV-005 | Conversation Mode long-session stability smoke | Adds an automated diagnostics source-shape and synthetic lifecycle smoke for longer Conversation Mode runs plus a Windows 5-10 minute manual checklist. The guard verifies queue max remains `4`, queue pressure/full and `drain_complete` remain visible, `queue_full` usable-audio overflow stays distinct from `empty_artifact` before-queue drops, lifecycle rows retain audio/drop/Owner Voice/candidate WAV cleanup fields, and Owner Voice dry-run remains non-blocking. The runtime selector remains the manual TASK-STT-005 selector; no STT default, runtime model selector behavior, queue policy, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit changed. IMPLEMENTED - automated long session diagnostics smoke PASS; needs Windows 5-10 min Conversation smoke. |
| TASK-CONV-006 | Conversation Mode backpressure pause/resume | Keeps pending queue max at `4` and adds a pause/resume throttle before new VAD-triggered utterance recorders start. Capture pauses at `pending=4/4`, or at high watermark `pending>=3/4` while a turn is active/processing, and resumes at `pending<=2/4` or `drain_complete`. Diagnostics expose `conversationBackpressurePaused`, `conversationBackpressureReason`, and `conversationBackpressureResumeReason`. Normal long sessions should now pause capture instead of repeatedly producing usable-audio `queue_full`; `queue_full` remains a visible hard fallback with `audio=usable_audio`, `dropStage=at_queue`. Empty artifacts remain distinct. Stop/drain remains graceful and final state should still be `pending=0/4`, `active=0`, `stopMode=drain_complete`. No STT default, STT model selector, `/chat` or mood schema, Owner Voice hard gate, TTS, frontend redesign, IPC, Pet Window, Output Queue, raw audio persistence, or generated artifact commit changed. IMPLEMENTED - automated backpressure smoke PASS; needs Windows long-session re-smoke. |
