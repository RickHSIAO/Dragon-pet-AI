...
| TASK-CONV-003 | Conversation Mode queue backpressure artifacts | Adds safe lifecycle classification for queue backpressure and duplicate finalization investigation. `queue_full` rows now include duration/blob/chunk facts and `audio=usable_audio`, `dropStage=at_queue`; `0B` artifacts drop before queue admission as `reason=empty_artifact`, `audio=empty_artifact`, `dropStage=before_queue`; duplicate recorder finalization is ignored without overwriting the first terminal lifecycle status. Queue limit remains `2`; real overflow remains visible. No STT default, Owner Voice hard gate, schema, IPC, Pet Window, Output Queue, raw audio persistence, path/transcript/centroid/embedding exposure, queue ordering, graceful Stop/drain, or no-parallel `/chat` behavior change. IMPLEMENTED - automated backpressure smoke PASS; needs Windows runtime 4-turn re-smoke. |
...
- TASK-CONV-001 IMPLEMENTED - CONVERSATION MODE CONTINUOUS CAPTURE QUEUE (2026-06-05):
  ...
  The queue is bounded to 2 pending utterances and drops newest on overflow with `queue_full`.
...
- TASK-CONV-002 DONE - CONVERSATION MODE TURN LIFECYCLE DIAGNOSTICS (2026-06-11):
  ...
  local settings. This does not change TASK-CONV-001 queue ordering, max pending `2`, no-parallel
...
- TASK-CONV-003 IMPLEMENTED - CONVERSATION MODE QUEUE BACKPRESSURE ARTIFACTS (2026-06-11):
  ...
  `durationMs=0 bytes=0`. TASK-CONV-003 keeps max pending `2` and keeps real
  ...
  smoke PASS; Windows actual-audio 4-turn re-smoke remains required before DONE.
