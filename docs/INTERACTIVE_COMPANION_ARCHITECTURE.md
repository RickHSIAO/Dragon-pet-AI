...
- TASK-STT-006C IMPLEMENTED - AUTOMATED GROUNDED EXPLANATION SMOKE PASS / NEEDS WINDOWS EXPLANATION REPORT SMOKE (2026-06-12):
  ...
  `outputs/stt_model_explanation/YYYYMMDD/`. The flow is now backend
  evaluation report -> deterministic scoring report -> grounded explanation.
  `christina` and `plain` styles both use only scoring-report facts and include
  uncertainty/caveats. The current implementation is deterministic/no-LLM by
  default; `--use-llm` remains a future-compatible fallback path and does not
  call a live provider. Runtime selection remains manual through the existing
