...
- TASK-STT-006B IMPLEMENTED - AUTOMATED SCORING SMOKE PASS / NEEDS WINDOWS SCORING REPORT SMOKE (2026-06-12):
  ...
  under `outputs/stt_model_scoring/YYYYMMDD/`. The flow is evaluation report ->
  deterministic scoring report -> future AI explanation. Profiles are
  `manual_mic`, `conversation`, and `balanced`; the conversation profile weights
  speed more heavily. No true transcript accuracy/WER is claimed without
  reference transcripts. No LLM/AI explanation, frontend comparison panel,
  runtime auto-switch, default model change, `/chat` or mood schema change,
  Owner Voice hard gate, renderer IPC, committed audio, or generated report was
  added.
