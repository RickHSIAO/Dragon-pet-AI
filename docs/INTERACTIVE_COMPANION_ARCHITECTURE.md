...
| TASK-STT-004 | No-speech / silence hallucination guard | Adds conservative WAV PCM audio-energy diagnostics and faster-whisper segment metadata checks before accepting silent/near-silent STT output. Guarded results return `status=no_speech`, empty final transcript, and safe diagnostics; Manual Mic does not fill textarea or auto-send, and Conversation Mode does not enqueue/send chat while preserving rearm and graceful drain. Suspicious subtitle-credit phrases are suppressed only with strong no-speech evidence. No default model switch, `/chat` schema change, Owner Voice hard gate, raw audio persistence, private path exposure, or aggressive rewrite. |
