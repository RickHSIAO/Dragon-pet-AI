...
| TASK-STT-005 | Runtime STT model selection UI | Adds a session-only Full App `STT model` selector with `Default / env`, `tiny`, `base`, and `small`. `/stt/transcribe` accepts an optional multipart `model` field; empty/omitted keeps env/default resolution, valid request values use `modelSource=request`, and invalid request values fall back safely with `invalid_request_model`. Manual Mic and Conversation Mode use the existing `stt:transcribe` IPC bridge and send no model for `Default / env`. Diagnostics show UI selected model, request model sent, requested/resolved model, source, fallback reason, provider load state, and model load error. Runtime default remains `tiny`; automated model-selection smoke PASS; Windows runtime model-switch smoke still required. No `/chat` or mood schema change, Owner Voice hard gate, IPC channel change, queue/pre-roll/drain change, or raw audio persistence. |
...
- TASK-STT-005 IMPLEMENTED - AUTOMATED MODEL SELECTION SMOKE PASS / NEEDS WINDOWS RUNTIME MODEL SWITCH SMOKE (2026-06-12):
  ...
  fallback reason, provider load state, and model load error. No `/chat` or
  mood schema change, Owner Voice hard gate, IPC channel change, Conversation
  queue/pre-roll/drain policy change, raw audio persistence, or default model
  switch was added.
