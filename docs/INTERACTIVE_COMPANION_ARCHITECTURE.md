...
| TASK-PERSONA-002 | Christina general tone sanitizer | Adds positive non-debug prompt guidance for proud, teasing, witty, protective replies without contempt or hostile unclear-input dismissal. Keeps TASK-PERSONA-001 debug repair first, then applies narrow general repair for known harsh fragments, repeated humiliation, unclear-input hostility, and likely garbled-STT cases. Safe tsundere lines pass through. No `/chat` schema, mood schema, STT default, Owner Voice hard gate, renderer IPC, provider runtime, or runtime model change. IMPLEMENTED - prompt/repair smoke PASS; needs Windows general tone smoke. |
...
framing, and repeated humiliation while preserving proud tsundere flavor. The
...
hostility, repeated humiliation, or likely garbled-STT hostility. This does not
change `/chat` response shape, mood schema, renderer IPC, STT default, Owner
