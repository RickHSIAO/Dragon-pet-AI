...
- Security boundary prerequisites before Owner Voice Gate runtime wiring are now
  ...
  and TASK-267 adds Conversation Mode dry-run status only. Both keep existing
  runtime behavior non-blocking. Future owner voice hard-gate work remains
  deferred and must stay opt-in, disabled by default, and policy gated before
