...
| TASK-PERSONA-001 | Christina tsundere tone boundary | Tunes the runtime persona prompt to keep proud / tsundere / protective Christina flavor while discouraging repeated direct humiliation, hostile name-calling, repeated address labels, and dismissive technical-debug responses. Third-pass tuning marks `又想試吾的耐心` disallowed for technical/debug mode, keeps `汝這傢伙` rare/non-consecutive, and requires broad debug fallback replies to give PASS / FAIL / NEEDS EVIDENCE, evidence needed, and next check. No `/chat` schema, mood schema, STT, Owner Voice, Ollama provider, renderer UI, IPC, or runtime model change. IMPLEMENTED - third prompt/static smoke PASS; needs Windows chat tone re-smoke. |
...
the recycled `又想試吾的耐心` debug line is disallowed; emotional/stress contexts
should reduce harshness; repeated direct insults, degrading labels, and repeated
address phrases such as consecutive `汝這傢伙` should be avoided. This changes only
prompt content and static tests; it does not change `/chat` response shape, mood
schema, renderer UI, IPC, STT, Owner Voice, Ollama provider behavior, or runtime
model defaults.
