...
| TASK-239 | Extract Diagnostics Drawer Module | runtime extraction (renderer-only, no behavior change) — **DONE - WINDOWS VISUAL SMOKE PASS / DONE - PASS (2026-06-02, Windows visual smoke 2026-06-01)**. Moves diagnostics drawer engine (~140 lines) from `renderer.js` into `modules/diagnostics-drawer.js` IIFE exposing `window.dragonDiagnosticsDrawer`. `_expanded` state moves to module. `renderInteractionReactionPreview` in `renderer.js` passes explicit state context to module. `index.html` loads module after output-queue.js, before renderer.js. 14 new smoke tests; 4 existing tests updated. All 3 smoke scripts PASS. Windows visual smoke PASS. Queue remains diagnostics ledger / disabled — no dispatch, no IPC, no Pet Window side effect, no behavior change. |
