...
7. TASK-244 Voice Quality Diagnostics / VAD Tuning. **IMPLEMENTED - NEEDS WINDOWS VOICE QUALITY SMOKE (2026-06-03).** Collapsible `<details>` diagnostics panel; session-only RMS threshold + silence duration tuning controls; `<pre>` display updated via `textContent` only; diagnostics state var and 4 helpers (`makeSafeTranscriptPreview`, `renderFullAppVoiceDiagnostics`, `updateFullAppVoiceDiagnostics`, `resetFullAppVoiceDiagnosticsForRecording`); session tuning vars (`fullAppConversationRmsThreshold`, `fullAppConversationSilenceMs`) used by `_conversationVadTick`; diagnostics patches in all recording/transcription/VAD functions. 22 TASK-244 smoke tests PASS (522 renderer-chat total). Windows voice quality smoke NEEDED.
