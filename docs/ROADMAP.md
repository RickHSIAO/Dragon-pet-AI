...
**Next planned task:** TASK-172A-OCR-POLISH IMPLEMENTATION (OCR Preprocessing)

- TASK-172A-OCR-POLISH DEFINED (docs-only) (2026-05-30): OCR Image Preprocessing / Quality Improvement (v0.4 screen context slice 2A-OCR-POLISH). Designs preprocessing pipeline for backend pytesseract OCR to improve output quality on full-desktop screenshots. Pipeline: grayscale → 2× upscale (LANCZOS, capped at 6000px) → contrast enhance (1.5×) → sharpen → optional binarization. Tesseract config: --psm 11 (sparse text) --oem 3 --dpi 150 as default. Language: eng+chi_tra if installed, falls back to eng only if chi_tra missing. Output cleanup: drop symbol-only lines, collapse repeated chars, preserve code/URLs/paths, re-bound to 800 chars; safety fallback if cleanup removes >60% of lines. All preprocessing in memory (io.BytesIO); no disk write; no external upload; no /chat call. Error table covers 7 failure cases with clean zh-TW messages. Testing plan: 8 backend pytest additions. No new desktop smoke tests (existing backend OCR tests cover flow). No runtime files modified in this docs-only step.
