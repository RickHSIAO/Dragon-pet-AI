...
**Next planned task:** TASK-174 — Capture current mouse display / active monitor (implementation)

- TASK-174 DEFINED (docs-only) (2026-05-30): Capture Current Mouse Display / Active Monitor Design (v0.4+ screen context capture improvement). On "擷取螢幕" click, main process calls `screen.getCursorScreenPoint()` and `screen.getDisplayNearestPoint()` to identify the display under the cursor, then matches against `desktopCapturer` source by `display_id`. Matched source is used for capture; single-source fallback to `sources[0]` remains safe; multi-source mismatch returns clean error `current-display-ambiguous` rather than silently using `sources[0]`. All existing TASK-171A safety rules preserved. No new IPC channels, no `nodeIntegration` change, no renderer access to raw source/display IDs. Existing OCR ("分析這張"), OCR backend, OCR polish, and `/chat` handoff ("問克莉絲蒂娜這個畫面") flows unchanged. Full error table covers 7 cases; all mapped to clean zh-TW messages. Testing plan: static source checks + dynamic mocks for cursor display matching + manual smoke (mouse on primary → primary content only; mouse on secondary → secondary content only). No runtime files modified in this docs-only step.
