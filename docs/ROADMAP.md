...
- TASK-166 DEFINED: Pet Overlay Shell Polish Design (v0.2). Designs 7 areas of overlay shell improvement: (1) transparent/frameless overlay presentation with character-readable contrast and fallback; (2) always-on-top edge case audit + re-assertion strategy without focus stealing; (3) drag/position behavior — no change, audit only; (4) scale presets (Small 225×300 / Medium 300×400 / Large 375×500) with URL-param delivery, merge-write persistence, and silent fallback; (5) click-through toggle with hover-strip recovery, forced-off during bubble/menu, in-memory only (no persist); (6) bubble placement with CSS speech-bubble tail pointing toward character; (7) control accessibility guarantees. Includes implementation split recommendation (166a–166e) and preserved-behavior table (TASK-148–TASK-162). No voice, no screen capture, no Live2D, no backend changes.

- TASK-165 DONE - PASS: Pet Mode v0.2 Companion Roadmap / Capability Reset. Docs-only roadmap reset separating v0.1 (DONE) from the full desktop AI companion vision. Defines v0.2 (Overlay Shell), v0.3 (Voice Interaction), v0.4 (Screen Context), v0.5 (Proactive Companion), and v0.6 (Live2D) target scopes. Recommends TASK-166 (Pet Overlay Shell Polish Design) as the next implementation task, with rationale for why overlay shell comes before voice and Live2D.

- TASK-164 DONE - PASS: Pet Mode v0.1 Demo / Portfolio Packaging. Docs-only task to package Pet Mode v0.1 (TASK-148 through TASK-163) for demo and portfolio use. Covers: one-paragraph project summary, problem statement, v0.1 feature list, demo flow script, screenshots to capture, video recording checklist, technical architecture summary (Electron + FastAPI + Ollama + IPC bridge + local state), safety/reliability highlights, testing/validation highlights, README/demo section suggestions, and portfolio talking points. No runtime changes; no new assets; no actual recording or screenshot capture.

- TASK-163 DONE - WINDOWS MANUAL SMOKE PASS: Pet Mode Release Checkpoint / Regression Smoke. End-to-end regression checklist covering all TASK-148 through TASK-162 Pet Mode behaviors: window lifecycle, position persistence, clean bubble speech, details disclosure, Ollama reliability, thinking sanitization, TASK-157 thinking bubble transition, success/error replacement, mood richness, mood expression mapping, idle rotation, idle timing/noise-control cooldown, Quiet Mode ON/OFF, Quiet Mode persistence, corrupt-value fallback, and no diagnostic leakage. Automated: 83 pet-renderer smoke tests, 20 pet-window smoke tests, renderer chat smoke, 619 pytest, git diff --check. Windows manual: 17-item checklist. No new runtime behavior; docs-only definition.
...
- TASK-160 DONE - WINDOWS MANUAL SMOKE PASS / DONE - PASS: Pet Quiet Mode / Idle Presence Toggle. Added `quietMode: false` to `presenceState`; `getQuietMode` / `setQuietMode` helpers; `getPetQuietMode` / `setPetQuietMode` public functions; `startIdleRotation` early-returns when `quietMode === true`; `pet-menu-quiet-mode` button in `pet.html` menu. Bug fix (2026-05-26): added `setIdleQuietBubble` helper — when quiet ON, collapses bubble to `collapsed` (hidden) instead of showing static `idle_default` text; updated `setPetIdleDefault`, `expireRecentPetReply`, `restorePetPresence` to call `setIdleQuietBubble`; `setQuietMode` ON collapses `idle_default` immediately, OFF restores `idle_default` + cooldown + rotation. 1 new smoke test added (`testQuietModeOnCollapsesIdleBubble`); 4 existing TASK-160 tests updated to assert collapsed state. 77/77 smoke tests PASS. All TASK-157/158/159 behaviors preserved. Frontend-only; no backend, IPC, preload, or schema changes. Windows manual smoke passed 2026-05-26 — 16/16 PASS (including bubble-collapse verification).

- TASK-159 DONE - WINDOWS MANUAL SMOKE PASS / DONE - PASS: Pet Idle Presence Timing / Noise Control Polish. Added `PET_IDLE_LAUNCH_QUIET_MS` (120 s) and `PET_IDLE_COOLDOWN_MS` (90 s). First idle rotation tick suppressed for 2 minutes after launch; idle rotation delayed 90 s after any activity (reply expiry, restore/show). `startIdleRotation` uses `delay = max(cooldownRemaining, PET_IDLE_ROTATION_MS)` so standard 60 s cadence is always respected. `setIdleCooldown` helper records deadline in `presenceState.idleCooldownUntil`. All 66 pet-renderer-smoke tests pass (7 new TASK-159 tests added). TASK-158 cycling and TASK-157 thinking behaviors preserved. Frontend-only; no backend, IPC, preload, or schema changes. Windows manual smoke passed 2026-05-26 — 17/17 PASS.

- TASK-158 DONE - PASS: Pet Idle Presence Line Rotation Polish. When the Pet is in `idle_default` (no recent reply, no thinking state, no pending chat, no active error), it periodically rotates through a static local set of 10 in-character idle lines every 60 seconds. Implementation: `PET_IDLE_ROTATION_MS = 60000`, `PET_IDLE_LINES` (10 lines: neutral / tsundere / sleepy / focused / companion), `pickNextIdleLine` (deterministic sequential cycling), `isIdleRotationEligible` (guards: recentReply, handoffActive, petChatPending, blocked bubble states: llm_local_error/backend_offline/timeout/thinking/pending/handoff), `startIdleRotation` / `stopIdleRotation` (timer lifecycle via existing `schedulePresenceTimer` / `clearPresenceTimer` pattern), `presenceState.idleRotationTimer` and `presenceState.lastIdleLineIdx` fields added to `createPetPresenceState`. Call sites: `setPetIdleDefault` starts rotation; `expireRecentPetReply` restarts rotation when returning to idle; `rememberRecentPetReply` stops rotation when a real reply arrives. Idle lines use `setBubbleState(doc, 'idle_default', { response: line })` — NOT routed through `rememberRecentPetReply`, so idle lines are never stored as `presenceState.recentReply` and are not restored on hide/show. Smoke coverage: 9 new tests in pet-renderer-smoke.js (59 total PASS). Frontend-only; no backend calls, no LLM-generated text, no new assets, no IPC changes, no layout redesign. Acceptance criteria and manual Windows smoke expectations defined in docs/TASKS.md.
