...
**Next planned task:** _(TASK-167B defined — Voice STT Transcription Design, pending implementation)_

- TASK-167B DEFINED (2026-05-28): Voice STT Transcription Design (v0.3 slice 1B). Implements `transcribeAudioBlob(blob) -> Promise<string>` — the TASK-167A stub — as a real STT transcription call. Preferred path: narrow `POST /stt/transcribe` backend endpoint using local `faster-whisper` or `openai-whisper`; no external API, no API key required. Audio Blob sent to backend in-memory only after explicit recording stop; no disk persistence; no hidden listening. Returns empty string on silence/low-confidence/invalid Blob; non-empty trimmed transcript on success. Pet shows distinct `"transcribing"` state (`data-transcribing="true"`) while STT runs, visually distinct from recording dot (TASK-167A) and thinking bubble (TASK-157). Empty result returns Pet to idle silently; non-empty result surfaces transcript for user confirmation (chat handoff deferred to TASK-167C). Cancel during transcription discards Blob silently. Error table: STT unavailable, timeout (15 s), silent audio, invalid Blob, backend offline — all clean Pet Bubble fallbacks, no stack traces. Click-through remains OFF during transcription. Quiet Mode suppresses idle only — not transcription. Text input / voice transcription mutual exclusion preserved. Scope: no TTS, wake word, always-listening, screen capture, OCR, Live2D, broad provider architecture, or raw audio persistence. One narrow backend endpoint only — no schema/config changes. Preserved-behavior table covers TASK-149–TASK-167A.
