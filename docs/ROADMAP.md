...
- TASK-146 implementation keeps Pet display-only: Chat now hands off to Full App, Full App keeps the existing focus/open path, Menu owns Toggle Details, Reset Pet Position, and Hide Pet Window, floating `i` was removed, `x` hides Pet Window through existing Hide Pet behavior, and no backend, `/chat`, IPC/preload, provider, asset, external API, image, or voice change was made.
